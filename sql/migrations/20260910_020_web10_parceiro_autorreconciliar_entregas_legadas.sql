-- IAGO Shopping - WEB-10 Adendo complementar
-- Autorreconciliacao segura para a area do parceiro.
--
-- Resolve pedidos pagos antigos que ganharam parceiro/repasse depois da aprovacao:
-- quando o parceiro abre "Meus pedidos", o frontend chama esta rotina antes de listar.
-- A rotina e idempotente, escopada em auth.uid(), nao reprocessa pagamento e nao move estoque.

drop function if exists public.shopping_parceiro_reconciliar_pedidos_pagos_legados();
  ...
  loop
    ...
    if v_snapshot_preenchido or v_entrega_criada then
      ...
      values (
        ...
        'Autorreconciliacao WEB-10 acionada pela area do parceiro, sem reprocessar pagamento ou estoque.'
      );
    end if;
...
comment on function public.shopping_parceiro_reconciliar_pedidos_pagos_legados() is
  'Autorreconcilia entregas faltantes de pedidos pagos legados para o parceiro autenticado, sem reprocessar pagamento ou estoque.';
